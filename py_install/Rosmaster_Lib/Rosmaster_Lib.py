...
class Rosmaster(object):
    ...
    def close(self):
        ...
        try:
            self.__uart_state = 0
            time.sleep(0.1)  # 스레드가 종료될 시간을 줍니다
    ...
    def __receive_data(self):
        # 清空缓冲区
        self.ser.flushInput()
        while self.__uart_state == 1:
            try:
                data = self.ser.read(1)
                if len(data) == 0:
                    continue
                head1 = bytearray(data)[0]
                if head1 == self.__HEAD:
                    data = self.ser.read(1)
                    if len(data) == 0:
                        continue
                    head2 = bytearray(data)[0]
                    check_sum = 0
                    rx_check_num = 0
                    if head2 == self.__DEVICE_ID - 1:
                        ...
                        if len(data) == 0:
                            continue
                        ext_len = bytearray(data)[0]
                        data = self.ser.read(1)
                        if len(data) == 0:
                            continue
                        ext_type = bytearray(data)[0]
                        ext_data = []
                        ...
                        if len(ext_data) == data_len and check_sum % 256 == rx_check_num:
                ...
                if self.__debug:
                    print("Serial exception in receive thread:", str(e))
                break
            except Exception as e:
                if self.__debug:
                    print("Error in receive thread:", str(e))
                break
        if self.__debug:
